...
def start_message(message):
    bot.send_message(message.chat.id, 'Welcome! Click the "Generate email" button to generate your temporary email. Click the "Refresh inbox" button to check your e-mail.', reply_markup=keyboard)
...
def send_text(message):
    if message.text.lower() == 'generate email':
        ...
        try:
            data = requests.get(
                "https://www.1secmail.com/api/v1/?action=getMessages&login="+email[:email.find("@")]+"&domain="+email[email.find("@")+1:]).json()
            if 'id' in data[0]:
                for i in range(len(data)):
                    ...
                    bkeyboard.add(InlineKeyboardButton(text=str(subject)+"\n from: "+ fromm +" in "+"[id"+str(id)+"]["+str(email)+"]"))
                    bot.send_message(message.chat.id,"Message Subject: "+subject+" From: "+fromm+" Date:"+date, reply_markup=bkeyboard)
                    count = i + 1
                bot.send_message(message.chat.id, "A total of "+str(count)+" messages found\nClick on the button to read the message")
            else:
